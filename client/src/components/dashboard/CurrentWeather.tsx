...
const CurrentWeather = ({ data, isLoading, className = "" }: CurrentWeatherProps) => {
  ...
  return (
    <Card className={`shadow-md ${className}`}>
      <CardContent className="p-6">
        <h3 className="text-lg font-heading font-medium mb-4 truncate">Current Weather in Hanoi</h3>
        <div className="flex flex-col md:flex-row items-center">
          <div className="text-center md:text-left md:mr-6 mb-4 md:mb-0 w-full">
            <div className="flex items-center justify-center md:justify-start">
            ...
            <p className="text-neutral-500 truncate max-w-full">{data.current.weatherDescription}</p>
            <p className="text-sm text-neutral-600 mt-2 truncate">Feels like {Math.round(data.current.feelsLike)}°C</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 flex-grow">
            <div className="flex items-center">
              ...
              <div className="overflow-hidden">
                <p className="text-sm text-neutral-500 truncate">Humidity</p>
                <p className="font-medium truncate">{data.current.humidity}%</p>
              </div>
            </div>
            <div className="flex items-center">
              <span className="material-icons text-neutral-500 mr-2">air</span>
              <div className="overflow-hidden">
                <p className="text-sm text-neutral-500 truncate">Wind</p>
                <p className="font-medium truncate">{data.current.windSpeed} km/h</p>
              </div>
            </div>
            <div className="flex items-center">
              <span className="material-icons text-neutral-500 mr-2">thermostat</span>
              <div className="overflow-hidden">
                <p className="text-sm text-neutral-500 truncate">Pressure</p>
                <p className="font-medium truncate">{data.current.pressure} hPa</p>
              </div>
            </div>
            <div className="flex items-center">
              <span className="material-icons text-neutral-500 mr-2">visibility</span>
              <div className="overflow-hidden">
                <p className="text-sm text-neutral-500 truncate">Visibility</p>
                <p className="font-medium truncate">{(data.current.visibility / 1000).toFixed(1)} km</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6">
          <h4 className="text-sm font-medium text-neutral-600 mb-3 truncate">Today's Forecast</h4>
                  ...
                  if (hour >= nextHour || time.getDate() > new Date().getDate()) {
                    ...
                    hourlyItems.push(
                      <div key={timeStr} className="flex flex-col items-center mx-auto min-w-[60px] px-2">
                        <p className="text-sm text-neutral-500 truncate w-full text-center">{displayHour}</p>
                        <span className="material-icons text-lg my-2">{icon}</span>
                        <p className="text-sm font-medium truncate w-full text-center">{temp}°C</p>
                      </div>
                    );
