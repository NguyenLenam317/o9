...
const AIChatbot = ({ userProfile }: AIChatbotProps) => {
  ...
  // Fetch existing chat history with device ID
  ...
  useEffect(() => {
    // Initialize WebSocket connection with device ID
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const host = window.location.host;
    
    // Create WebSocket with device ID in URL or use a custom header
    const ws = new WebSocket(`${protocol}//${host}/ws`);
    
    // Add device ID to the WebSocket connection during handshake
    ws.onopen = () => {
      console.log('WebSocket connected with device ID:', deviceId);
      
      // Send device ID as first message for identification
            ...
            <div className="flex flex-col h-[450px]">
              <div className="flex-grow bg-neutral-50 rounded-lg p-4 mb-4 overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-2 overflow-y-auto max-h-[400px] px-2 py-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {suggestions.map((suggestion, index) => (
                    <Button 
                      key={index}
                      variant="outline"
                      className="justify-start h-auto py-3 text-left hover:bg-primary/10 overflow-hidden text-ellipsis whitespace-nowrap"
                      onClick={() => {
                        setInput(suggestion);
                        setActiveTab('chat');
                      }}
