  ...
  return (
    <Card className="shadow-md">
      <CardContent className="p-6">
        ...
        <div className="space-y-4">
          <div className="p-4 bg-neutral-50 rounded-lg">
            <h4 className="font-medium mb-2 truncate">7-Day Outlook</h4>
            <div className="space-y-3">
              {predictionData.forecasts.map((forecast, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="truncate max-w-[150px]">{forecast.displayDate}</span>
                  </div>
                  <div className="flex items-center">
                    <Badge className={`${getRiskBadgeClass(forecast.risk)} truncate max-w-[100px]`}>{forecast.risk}</Badge>
                  </div>
                </div>
              ))}
            </div>
          ...
          {predictionData.event && (
            <div className="bg-warning bg-opacity-10 border-l-4 border-warning rounded-lg p-4">
              <div className="flex items-start">
                ...
                <div>
                  <p className="font-medium truncate max-w-[250px]">{predictionData.event.title}</p>
                  <p className="text-sm text-neutral-700 mt-1 truncate max-w-[300px]">{predictionData.event.description}</p>
                  <div className="mt-2">
                    <Button variant="link" className="text-sm text-primary font-medium p-0 h-auto truncate max-w-[200px]">
          ...
          {predictionData.personalImpact && predictionData.personalImpact.length > 0 && (
            <div>
              <h4 className="font-medium mb-2 truncate">Personalized Impact</h4>
              ...
              <ul className="list-disc list-inside space-y-1 text-sm text-neutral-700 mt-2">
                {predictionData.personalImpact.map((impact, index) => (
                  <li key={index} className="truncate max-w-[300px]">{impact}</li>
                ))}
              </ul>
            </div>
          )}
