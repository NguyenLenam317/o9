...
export interface IStorage {
  ...
  saveChatMessage(deviceId: string, content: string, role?: 'user' | 'ai'): Promise<ChatHistory>;
  getChatHistory(deviceId: string, limit?: number): Promise<ChatHistory[]>;
...
export class MemStorage implements IStorage {
  ...
  private deviceIdToUserMap: Map<string, number>; // Maps device IDs to user IDs
  ...
  constructor() {
    this.users = new Map();
    this.deviceIdToUserMap = new Map();
  ...
  async getUserByDeviceId(deviceId: string): Promise<User | undefined> {
    const userId = this.deviceIdToUserMap.get(deviceId);
    if (userId !== undefined) {
  ...
  async createUser(userData: InsertUser): Promise<User> {
    ...
    // Add deviceId support
    const user = { ...userData, id };
    this.users.set(id, user);
    
    // If the user has a deviceId, store the mapping
    if ((userData as any).deviceId) {
      this.deviceIdToUserMap.set((userData as any).deviceId, id);
    }
  ...
  async saveChatMessage(deviceId: string, content: string, role?: 'user' | 'ai'): Promise<ChatHistory> {
    // Implement saveChatMessage logic here
    // For now, just return an empty ChatHistory object
    return { id: 0, userId: 0, messages: [] };
  }

  async getChatHistory(deviceId: string, limit?: number): Promise<ChatHistory[]> {
    // Implement getChatHistory logic here
    // For now, just return an empty array of ChatHistory objects
    return [];
  }

  async clearDeviceHistory(deviceId: string): Promise<void> {
    // Implement clearDeviceHistory logic here
...
interface ChatMessage {
  id: string;
  deviceId: string; // Primary identifier for device isolation
  content: string;
  timestamp: string;
  role?: string; // 'user' or 'assistant'
}

// Store chat messages by device ID to ensure isolation
const chatStorage: {
  [deviceId: string]: ChatMessage[];
} = {};

// Simplified function that uses deviceId as the primary key
export async function saveChatMessage(deviceId: string, content: string, role: string = 'user') {
...
// Retrieve chat history by device ID
...
export function clearDeviceHistory(deviceId: string) {
  delete chatStorage[deviceId];
}

// Legacy function to maintain compatibility
export function clearSession(sessionId: string) {
  // In the new implementation, sessionId would be deviceId
